...
import './Navbar.css'
import Homepage from './CourseInstructor/HomeInstuctor';
import {Redirect} from 'react-router-dom'
import StaffAM from './CourseInstructor/StaffAM';
import NotificationsAM from './NotificationsAM';
import Logout from './logout';
import ViewProfileAM from './CourseInstructor/ViewProfileCI';
...
class Navbar extends Component{
    ...
    HomeAM(event){
        this.setState({RedirectToHomeAM:"/HomeInstructor"})
    ...
    ProfileAM(event){
        this.setState({RedirectToProfile:"/ViewProfileCI"})
        event.preventDefault()
    }
