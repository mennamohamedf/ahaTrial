...
export default class StaffLeavesHR extends Component {
    ...
    acceptreq(event){
        ...
        axios.post('/AcceptAccidentalLeaveHR', inputData, {headers: {
    ...
    rejectreq(event){
        ...
        axios.post('/RejectAccidentalLeaveHR', inputData, {headers: {
            'x-auth-token': localStorage.getItem('savedToken')
        }}).then(response =>{
            console.log(response.data);
            //maybe a pop up message with response.data
   ...
   componentDidMount= ()=>{
       axios.get('/viewStaffRequestsHRaccidental', {headers: {
        'x-auth-token': localStorage.getItem('savedToken')
    }}).then(response =>{
