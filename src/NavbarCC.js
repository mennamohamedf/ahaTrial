...
import './Navbar.css'
import Homepage from './CourseCoordinator/CoordinatorHomeCont';
import {Redirect} from 'react-router-dom'
import StaffAM from './CourseInstructor/StaffAM';
import NotificationsAM from './NotificationsAM';
import Logout from './logout';
import ViewProfileAM from './CourseCoordinator/ViewProfileCC';
...
class Navbar extends Component{
    ...
    HomeAM(event){
        this.setState({RedirectToHomeAM:"/HomeC"})
    ...
    ProfileAM(event){
        this.setState({RedirectToProfile:"/ViewProfileCC"})
        event.preventDefault()
    }
